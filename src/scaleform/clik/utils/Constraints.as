...
package scaleform.clik.utils
{
	...
	public class Constraints extends EventDispatcher
	{
		...
		public function addElement(name:String, clip:DisplayObject, edges:uint):void
		{
			...
			var element:ConstrainedElement = new ConstrainedElement(clip, edges, clip.x, clip.y, w / scope.scaleX - (clip.x + clip.width), h / scope.scaleY - (clip.y + clip.height), clip.scaleX, clip.scaleY);
		...
		{
			if (scaleMode == ConstrainMode.REFLOW)
			{
				return parentXAdjust;
			}
		...
		protected function addToParentConstraints():void
		{
			...
			while (p != null)
			{
				if (p.hasOwnProperty("constraints"))
				{
					...
					if (parentConstraints != null && parentConstraints.scaleMode == ConstrainMode.REFLOW)
					{
						return;
					}
					else if (parentConstraints != null && scaleMode == ConstrainMode.REFLOW)
					{
						return;
					} // NFM: 10/10: Added for TextArea.
					if (parentConstraints != null)
					{
