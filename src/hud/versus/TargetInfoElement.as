package hud.versus
{
	...
	public class TargetInfoElement extends BaseControl
	{
		...
		{
			if (param1.nextTargetTime == 0)
			{
				this.resetToInitialState();
				return;
			}
			this.m_headerString = param1.isKiller == true ? Localization.get("UI_HUD_VS_NEXT_TARGET_TIMER") : Localization.get("UI_HUD_VS_KILL_TARGET_TIMER");
			if (param1.isDuelStart == true)
			{
				this.m_headerString = "";
			}
			this.m_counterString = param1.nextTargetTime.toString();
			this.updateHeader();
			if (this.m_animateIn)
			{
		...
		{
			Animate.fromTo(this.m_headerClip, 0.3, 0, {"alpha": 0, "y": this.m_headerPosY + 30}, {"alpha": 1, "y": this.m_headerPosY}, Animate.ExpoOut);
			Animate.fromTo(this.m_counterClip, 0.15, 0.3, {"alpha": 0}, {"alpha": 1}, Animate.ExpoOut);
			Animate.addFromTo(this.m_counterClip, 0.3, 0.3, {"scaleX": 0, "scaleY": 0}, {"scaleX": 1, "scaleY": 1}, Animate.ExpoOut);
		}
		
		private function animateCount():void
		{
			Animate.fromTo(this.m_counterClip, 0.2, 0, {"scaleX": 1.5, "scaleY": 1.5}, {"scaleX": 1, "scaleY": 1}, Animate.ExpoOut);
			this.updateCounter();
		}
