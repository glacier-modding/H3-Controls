...
package scaleform.clik.motion
{
	...
	public class Tween
	{
		...
		public function Tween(duration:Number, target:Object = null, props:Object = null, quickSet:Object = null)
		{
			...
			if (quickSet == null || quickSet.paused == null)
			{
				this.paused = false;
			}
		}
		...
		public function set paused(value:Boolean):void
		{
			...
			{
				if (firstTween)
				{ // Insert this tween at the front of the tween list.
					firstTween.prev = this;
					next = firstTween;
				}
				firstTween = this;
				if (_position >= delay + duration)
				{
					_position = 0;
				}
			}
		...
		protected function init():void
		{
			...
			if (useMatrix)
			{
				...
				deltaMatrix = new Matrix(isNaN(props.scaleX) ? 0 : props.scaleX - startMatrix.a, isNaN(props.rotation) ? 0 : (props.rotation - targetDO.rotation) * degToRad, isNaN(props.alpha) ? 0 : props.alpha - startMatrix.c, isNaN(props.scaleY) ? 0 : props.scaleY - startMatrix.d, isNaN(props.x) ? 0 : props.x - startMatrix.tx, isNaN(props.y) ? 0 : props.y - startMatrix.ty);
			}
			props = null;
		}
		...
		protected function updatePosition(value:Number):void
		{
			...
			if (ease != null)
			{
				ratio = (easeParam == null) ? ease(ratio, 0, 1, 1) : ease(ratio, 0, 1, 1, easeParam);
			}
			...
			if (startMatrix)
			{
				...
				if (deltaMatrix.c)
				{
					targetDO.alpha = startMatrix.c + deltaMatrix.c * ratio;
				}
			}
