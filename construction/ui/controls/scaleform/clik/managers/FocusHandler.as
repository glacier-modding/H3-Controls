...
package scaleform.clik.managers
{
	...
	public class FocusHandler
	{
		...
		public function setFocus(focus:InteractiveObject, focusGroupIdx:uint = 0, mouseChange:Boolean = false):void
		{
			...
			if (focus != null)
			{ // New focus can be null if clicking on something that is not focus enabled.
		...
		protected function handleInput(event:InputEvent):void
		{
			...
			{
				component = _stage;
			} // If nothing is selected, dispatch input from the stage?
			...
			{
				return;
			} // Only key-down events have a default behaviour
			var nav:String = event.details.navEquivalent;
			if (nav == null)
			{
				return;
			} // Only navigation equivalents have a default behaviour.
			...
			// TextField edge case
			if (actualFocus is TextField && actualFocus == focusedElement && handleTextFieldInput(nav, controllerIdx))
			{
				return;
			}
			if (actualFocus is TextField && handleTextFieldInput(nav, controllerIdx))
			{
				return;
			}
		...
		protected function updateActualFocus(event:FocusEvent):void
		{
			...
			{ // NFM: Should this only be for FOCUS_OUT? (Probably, but could use review.)
